...
export const NewTableRow: FunctionalComponent<NewTableRowProps> = ({ entity, onCreate }) => {
    ...
    return (
        <tr class="group table table-fixed w-full border-b bg-gray-800 border-gray-700 hover:bg-gray-600">
            {fields.map(field => <td class="px-6 py-4">
                {editMode ? <input disabled={field[0] === 'id'} style={{ width: field[0].length + 'ch' }} class={`w-40  ${field[0] === 'id' ? 'disabled bg-gray-800 group-hover:bg-gray-600' : ''}`} value={field[1]} onInput={(e) => onFieldChange(field[0], e.currentTarget.value)} /> : field[1]}
            </td>)}
